...
class SignUpController @Inject() (
  ...
  //val smsComponent = new SMSComponent(ws)
  ...
  def smsSignUp = Action.async { implicit request =>
    SignUpForm.form.bindFromRequest.fold(
      ...
      data => {
        ...
        userService.retrieve(loginInfo).flatMap {
          case Some(user) =>
            Future.successful(Redirect(routes.ApplicationController.signUp()).flashing("error" -> "user.exists"))
          case None =>
            ...
            for {
              ...
              user <- userService.save(user.copy(avatarURL = avatar))
              authInfo <- authInfoRepository.add(loginInfo, authInfo)
              authenticator <- env.env.authenticatorService.create(loginInfo)
              value <- env.env.authenticatorService.init(authenticator)
              result <- env.env.authenticatorService.embed(value, Redirect(routes.Application.main()))
            } yield {
              env.env.eventBus.publish(SignUpEvent(user, request))
              env.env.eventBus.publish(LoginEvent(user, request))
              result
            }
        }
      }
