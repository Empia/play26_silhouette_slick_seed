 ...
 */
class OpenIDInfoDAO @Inject() (userDAO: UserDAOImpl)(protected val dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext)
  extends DelegableAuthInfoDAO[OpenIDInfo] {

  val dbConfig = dbConfigProvider.get[JdbcProfile]
  val db = dbConfig.db
  import dbConfig.profile.api._
  private[models] val slickOpenIDInfos = TableQuery[OpenIDInfos]
  private[models] val slickOpenIDAttributes = TableQuery[OpenIDAttributes]
  ...
  protected def addAction(loginInfo: LoginInfo, authInfo: OpenIDInfo) =
    userDAO.loginInfoQuery(loginInfo).result.head.flatMap { dbLoginInfo =>
      DBIO.seq(
  ...
  def save(loginInfo: LoginInfo, authInfo: OpenIDInfo): Future[OpenIDInfo] = {
    val query = userDAO.loginInfoQuery(loginInfo).joinLeft(slickOpenIDInfos).on(_.id === _.loginInfoId)
  ...
  def remove(loginInfo: LoginInfo): Future[Unit] = {
    ...
    val openIDInfoSubQuery = slickOpenIDInfos.filter(_.loginInfoId in userDAO.loginInfoQuery(loginInfo).map(_.id))
  ...
  private[models] class OpenIDInfos(tag: Tag) extends Table[DBOpenIDInfo](tag, "openidinfo") {
    def id = column[String]("id", O.PrimaryKey)
    def loginInfoId = column[Long]("logininfoid")
    def * = (id, loginInfoId) <> (DBOpenIDInfo.tupled, DBOpenIDInfo.unapply)
  }

  private[models] class OpenIDAttributes(tag: Tag) extends Table[DBOpenIDAttribute](tag, "openidattributes") {
