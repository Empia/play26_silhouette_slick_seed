 ...
 */
class OAuth1InfoDAO @Inject() (userDAO: UserDAOImpl)(protected val dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext)
  extends DelegableAuthInfoDAO[OAuth1Info] {

  val dbConfig = dbConfigProvider.get[JdbcProfile]
  val db = dbConfig.db
  import dbConfig.profile.api._
  private[models] val slickOAuth1Infos = TableQuery[OAuth1Infos]
  ...
  protected def oAuth1InfoSubQuery(loginInfo: LoginInfo) =
    slickOAuth1Infos.filter(_.loginInfoId in userDAO.loginInfoQuery(loginInfo).map(_.id))
  ...
  def save(loginInfo: LoginInfo, authInfo: OAuth1Info): Future[OAuth1Info] = {
    val query = userDAO.loginInfoQuery(loginInfo).joinLeft(slickOAuth1Infos).on(_.id === _.loginInfoId)
  ...
  private[models] class OAuth1Infos(tag: Tag) extends Table[DBOAuth1Info](tag, "oauth1info") {
