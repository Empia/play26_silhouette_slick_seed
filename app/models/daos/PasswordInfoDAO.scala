 ...
 */
class PasswordInfoDAO @Inject() (userDAO: UserDAOImpl)(protected val dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext)
  extends DelegableAuthInfoDAO[PasswordInfo] {

  val dbConfig = dbConfigProvider.get[JdbcProfile]
  val db = dbConfig.db
  import dbConfig.profile.api._
  private[models] val slickPasswordInfos = TableQuery[PasswordInfos]
  ...
  protected def passwordInfoSubQuery(loginInfo: LoginInfo) =
    slickPasswordInfos.filter(_.loginInfoId in userDAO.loginInfoQuery(loginInfo).map(_.id))
  ...
  def save(loginInfo: LoginInfo, authInfo: PasswordInfo): Future[PasswordInfo] = {
    val query = userDAO.loginInfoQuery(loginInfo).joinLeft(slickPasswordInfos).on(_.id === _.loginInfoId)
  ...
  private[models] class PasswordInfos(tag: Tag) extends Table[DBPasswordInfo](tag, "passwordinfo") {
