 ...
 */
class OAuth2InfoDAO @Inject() (userDAO: UserDAOImpl)(protected val dbConfigProvider: DatabaseConfigProvider)(implicit ec: ExecutionContext)
  extends DelegableAuthInfoDAO[OAuth2Info] {

  val dbConfig = dbConfigProvider.get[JdbcProfile]
  val db = dbConfig.db
  import dbConfig.profile.api._
  private[models] val slickOAuth2Infos = TableQuery[OAuth2Infos]
  ...
  protected def oAuth2InfoSubQuery(loginInfo: LoginInfo) =
    slickOAuth2Infos.filter(_.loginInfoId in userDAO.loginInfoQuery(loginInfo).map(_.id))
  ...
  def save(loginInfo: LoginInfo, authInfo: OAuth2Info): Future[OAuth2Info] = {
    val query = for {
      result <- userDAO.loginInfoQuery(loginInfo).joinLeft(slickOAuth2Infos).on(_.id === _.loginInfoId)
    } yield result
  ...
  private[models] class OAuth2Infos(tag: Tag) extends Table[DBOAuth2Info](tag, "oauth2info") {
